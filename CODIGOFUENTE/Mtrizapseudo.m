...
%%% pseudo-esatico de un muro de retención en voladizo sección Cantilever por el metodo de MONOBE-OKABE.

%%% Entrdas: Base1: es la base 1 del muro de retención.  Base2: es la base 2 del muro de retención. Base3: es la base 3 del muro de retención. Co:es la corona del
...
function [FSD,FSVR,FSVA,FSDs,FSVRs,FSVAs,dEae]= Mtrizapseudo(Base1,Base2,Base3,Ha,ht,tal,D,B,alpha,phib,concreto,espesor,phi,PesoE,Cohesion,n,k,espe,pe,ph,R,TS,R1,R2,kh,kv,So)
...
%     assignin('base','dEae',dEae);
    ...
    nu=(PesoRes+pvertical)*tand(phib)+(c*(Base3));
    assignin('base','nu',nu);
    de=(phor+Pesoagua+dEae);
    assignin('base','de',de);
    FSDs=nu/de;
...
%                                 append(s,plot1);
                                ...
                                if alpha>0
                                    ...
                                    brazo=[round(Br(1),3),round(Br(2),3),round(Br(3),3),round(Br(4),3),roudn(bi,3)," "]';
                                    ...
                                    Tabla.Style= [Tabla.Style {NumberFormat("%.2f")}];
                                ...
                                if alpha==0
                                    ...
                                    brazo=[round(Br(1),3),round(Br(2),3),round(Br(3),3),round(Br(4),3)," "]';
                                ...
                                if R==1
                                     ...
                                     pesop=[Fp,Fpc]';
                                     brazop=[round(brap,3),""]';
                                     momentop=[round(Mp,3),"No se considera para el F.S, ya que cae sobre el punto de aplicación del momento"]';
...
%                                 append(s3,plot3);
                                ...
                                secc1=["Pav (Pa*sen(δ))","Pah (Pa*cos(δ))","Pagua"]';
